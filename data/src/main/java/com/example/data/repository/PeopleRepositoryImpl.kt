...
class PeopleRepositoryImpl(
    ...
    override fun getPeopleByEventId(eventId: Int): Flow<List<com.example.model.people.People>> {
        return flow {
            emit(value = generatePeopleList(count = 5).map { mapper.peopleResponseToPeople(it) })
        }.flowOn(Dispatchers.IO)
    }

    override fun getPeopleByCommunityId(communityId: Int): Flow<List<com.example.model.people.People>> {
        return flow {
